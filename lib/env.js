...
class Env extends Promise {
  constructor(ancestorEnv, ...args) {
    ...
    this.log = getApplog().child({ module: `[${this.class.name}]` });
  }
...
fs.readdirSync(path.join(__dirname, "subenv")).forEach(filename => {
  ...
  require(envPath)(Env);
  getSyslog().info("加载环境对象:", envPath);
});

// 处理 Env 的 unhandledRejection
process.on("unhandledRejection", (reason, p) => {
  if (p.superClass === Env) {
    getSyslog().error(reason, "未处理的 %s(%s) 对象异常", p.class.name, p._op.constructor.name);
  } else {
    getSyslog().error(reason, p);
  }
});
