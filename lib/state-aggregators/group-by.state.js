module.exports = states => {
  class Group {
    constructor(gid, groupNameList, state) {
      this.gid = gid
    ...
    async collect(elem) {
      return this.state.collect(elem)
    }
  }
  ...
  class GroupByStateAggregator extends states.StateAggregator {
    ...
    constructor(stateGenerator, ...assigners) {
      ...
      this.sep = "\r\n"
      this.assigner = (elem) => assigners.map(assigner => assigner(elem))
      this._result = new Map()
    }

    async collect(elem) {
      const groupNameList = this.assigner(elem)
      const gid = groupNameList.join(this.sep)
      let groupState = this._result.get(gid)

      if (!groupState) {
        groupState = new Group(gid, groupNameList, this.stateGenerator.createStateStore());
