...
class Mutex {
  ...
  async lockWithExpire() {
    try {
      ...
      return await tx.set(this.resourceLocation, this.STATE.OCCUPY, "ex", this.lockExpire).exec();
    } catch (err) {
  ...
  async acquire() {
    ...
    try {
      // 设置过期时间, 防止意外死锁.
      const ret = await this.lockWithExpire();
      if (!ret) {
        throw new Error(`资源 ${this.resourceId} 已被锁定`);
      }
