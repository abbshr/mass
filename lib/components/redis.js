...
class Redis extends RedisCache {
  ...
  onMessage(callback) {
    if (this.ready) {
      return this.client.on("message", callback);
  ...
  onceMessage(callback) {
    if (this.ready) {
      return this.client.once("message", callback);
    } else if (this.lastConnectionError) {
  ...
  offMessage(callback) {
    if (this.ready) {
      return this.client.off("message", callback);
    } else if (this.lastConnectionError) {
  ...
  offAllMessage() {
    if (this.ready) {
      return this.client.removeAllListeners("message");
    } else if (this.lastConnectionError) {
