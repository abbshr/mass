...
class MassElement {
  ...
  constructor(struct, sharable = true) {
    ...
    // new 模式
    Object.assign(this, {
      eventTime: eTime ? new Date(eTime) : new Date(),
      packedTime: new Date(),
      path: [],
      retries: 0,
    }, struct);
    ...
    if (struct.hasOwnProperty("record")) {
      if (typeof struct.record === "object") {
    ...
    switch (typeof data) {
      case "object":
        ...
        if (data === null) {
          ...
          return new this({ record: Array.from(data) });
        } else if (data instanceof Buffer) {
          return new this({ record: Buffer.from(data) });
        } else if (ArrayBuffer.isView(data)) {
          return new this({ record: new data.constructor(data) });
        } else if (data instanceof Map) {
          return new this({ record: new Map(data) });
        } else if (data instanceof Set) {
          return new this({ record: new Set(data) });
        } else if (data instanceof Date) {
          return new this({ record: new Date(data) });
        } else {
