...
const ops = {
  ...
  // 输出操作符
  Sink: require("../sinks/sink"),

  // 算子操作符
  Calculator: require("../calculators/calculator"),
};
...
const manager = {
  ...
  // 在所有操作符 (除了 Source/Sink/Calculator 之外) 使用前, 必须执行该方法完成加载
  // TODO: calculators
  loadall() {
    ["sources", "sinks"].forEach(dir => {
      const dirname = path.join(__dirname, "..", dir);
      fs.readdirSync(dirname).forEach(filename => require(path.join(dirname, filename)));
    });
  ...
  register(operatorName, OperatorClass) {
    ...
    console.log("已加载操作符", `<${operatorName}>`, "class:", OperatorClass.name);
  },
};
