...
class Calculator extends Transform {
  ...
  // 
  // get internalStateStore() {
  //   return this._state;
  // }

  // set internalStateStore(state) {
  //   if (!state instanceof this.env.State) {
  //     throw new Error("算子内部存储必须为 State 及其子类的实例");
  //   }
  //   this._state = state;
  // }
  ...
  createStateStore() {
    if (this.env.stateGenerator) {
      return new this.env.stateGenerator.createStateStore();
    } else {
