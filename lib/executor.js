...
class Executor {
  ...
  proceed(ret) {
    ...
    for (let subTask of task.childTasks) {
      if (Array.isArray(subTask)) {
        ...
        scheduler.schedule(new TaskClass({ ...cfg, scheduler: this.scheduler }), { ...ret });
      } else {
    ...
    for (let subTask of task.childTasksGroup) {
      if (Array.isArray(subTask)) {
        const [TaskClass, cfg] = subTask;
        ret.forEach(item => scheduler.schedule(new TaskClass({ ...cfg, scheduler: this.scheduler }), { ...item }));
      } else {
  ...
  async acquireMutex() {
    ...
    if (err) {
      ...
      // 如果已经上锁了, 日志等级仅为 info
      this.log[level](err, "Task <%s, %s> 未取得互斥锁 [%s]", task.taskId, task.taskName, task.resourceId);
      return false;
    }
