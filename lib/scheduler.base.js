...
class MassTaskSchedulerBase {
  ...
  preemptiveSchedule(priority, task, input) {
    this._sched(priority, task, "task-queue", input);
  }

  // 顺序调度
  schedule(task, queue, input) {
    this._sched(this.NICE, task, queue, input);
  }

  _sched(priority, task, queue, input) {
    if (task.canceled) {
      return;
    }

    let executor = task.createExecutor(input);
